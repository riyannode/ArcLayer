...
export const RUNNER_LOCAL_TOOLS: RunnerToolRegistryItem[] = [
  ...
  { name: "erc8183.set_budget", source: "runner-local", status: "active", risk: ["external-process"], capabilities: ["erc8183", "lifecycle"], roles: ["client", "provider", "devops-admin", "full-stack-agent"], requiresCircle: true, description: "Set budget for an ERC-8183 job" },
...
export const CONSOLE_MCP_PROXY_TOOLS: RunnerToolRegistryItem[] = [
  ...
  // Validation
  { name: "validation.request_calldata", source: "console-mcp-proxy", status: "active", risk: ["prepare-only"], capabilities: ["validation"], roles: ["validation-agent", "identity-agent"], description: "Request validation calldata" },
  { name: "validation.response_calldata", source: "console-mcp-proxy", status: "active", risk: ["prepare-only"], capabilities: ["validation"], roles: ["validation-agent", "evaluator"], description: "Response validation calldata" },
