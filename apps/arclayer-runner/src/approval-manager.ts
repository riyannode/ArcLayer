...
export class ApprovalManager {
  ...
  getApproval(approvalId: string, walletAddress: string, role: string): ApprovalRecord {
    ...
    // Validate wallet (unless reading as any role for list/get)
  ...
  async approve(args: ApproveArgs): Promise<{
    ...
    try {
      ...
      const result = await this.executeAction(approval.actionType, params);

      // Extract txHash and operationId from result
      const txHash = (result as Record<string, unknown>)?.txHash as string | undefined;
      const operationId = (result as Record<string, unknown>)?.operationId as string | undefined;
