...
function eventCategory(event: BridgeSession['events'][number]) {
  return event.category || categoryFromMetadata(event.metadata);
}
        ...
        <section className="rounded-sm border border-white/10 bg-black/25 p-4">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-[#EAE4D8]/60">Events for this category only when external runtimes include event.category or metadata.category.</p>
            </div>
