...
export default function AgentSetupPage() {
  ...
  const INSTALL_CMD = 'npx arclayer-codex@latest';
    ...
    <div className="min-h-screen overflow-hidden bg-[#050607] text-[#EAE4D8]">
      ...
      <main className="relative mx-auto max-w-2xl px-4 py-10">
        ...
        <section className="mb-6">
          ...
          <h1 className="mt-3 text-[28px] font-semibold tracking-[-0.04em] text-[#F4EFE5] md:text-[34px]">Install ArcLayer Codex MCP Connector</h1>
          <p className="mt-3 text-[13px] leading-6 text-[#EAE4D8]/62">Start with an MCP Agent Bundle for identity, manifest, role metadata, and ArcLayer API key readiness.</p>
        </section>
        ...
        <div className="mb-5 rounded-lg border border-white/10 bg-[#07090D]/88 px-7 py-5 shadow-[0_0_0_1px_rgba(0,0,0,0.25)]">
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg border border-[#C5A67C]/20 bg-[#C5A67C]/10 text-[#F0B84A]"><Code2 className="h-5 w-5" /></div><div><div className="font-mono text-[11px] uppercase tracking-[0.18em] text-[#F3C536]">Recommended</div><h2 className="mt-1 text-[18px] font-semibold tracking-[-0.03em] text-[#F4EFE5]">OAuth-ready Codex MCP connector</h2></div></div>
          <p className="mt-3 text-[13px] leading-5 text-[#EAE4D8]/62">Run this once on the device where Codex is installed:</p>
          
          <div className="mt-4 rounded-md border border-white/10 bg-black/35 px-4 py-3 font-mono text-[14px] text-[#EAE4D8] break-all">{INSTALL_CMD}</div>
          <div className="mt-4"><button type="button" onClick={handleCopy} className={`inline-flex h-10 items-center gap-3 rounded-lg border px-5 text-[12px] font-semibold transition ${copied ? 'border-[#B8CD7E]/40 bg-[#B8CD7E]/10 text-[#B8CD7E]' : 'border-[#F0B84A]/40 bg-[#F0B84A] text-black shadow-[0_0_34px_rgba(240,184,74,0.22)] hover:scale-[1.01] hover:bg-[#FFD084]'}`}>{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? 'Copied' : 'Copy command'}</button></div>

          <div className="mt-6 rounded-lg border border-[#F3C536]/25 bg-[#F3C536]/[0.055] p-5">
            <p className="text-[13px] leading-5 text-[#EAE4D8]/80">This installs the ArcLayer MCP configuration and Agent Bundle Skill for Codex.</p>
            <ul className="mt-4 space-y-2 text-[12px] text-[#EAE4D8]/58">
              <li>• The connector uses OAuth.</li>
              <li>• Codex can request wallet actions, but signing remains browser-mediated.</li>
              <li>• No private key leaves your browser.</li>
            </ul>
          </div>

          <p className="mt-5 text-[12px] leading-5 text-[#EAE4D8]/42">After authorization, ask Codex to create an Agent Bundle. Codex returns a browser mint URL; wallet signing remains in ArcLayer web. Bot runtime, Runner, payer wallet, Gateway balance, live ERC-8183 execution, and x402 execution happen later.</p>
        </div>
