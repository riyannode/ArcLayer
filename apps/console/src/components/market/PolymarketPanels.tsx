...
export function PolymarketBtc15mPanel({ snapshot, loading, error }: { snapshot: LiveSnapshot | null; loading?: boolean; error?: string | null }) {
  ...
  return <PanelShell title="Polymarket BTC 15m">
    ...
    {snapshot?.candleError ? <div className="mb-2 text-xs text-amber-300">Live candle source unavailable: {snapshot.candleError}</div> : null}
...
export function PolymarketStyleBtcChart({ snapshot, loading, error, onRefresh }: { snapshot: LiveSnapshot | null; loading?: boolean; error?: string | null; onRefresh?: () => void }) {
  ...
  const MAX_VISIBLE_CANDLES = 35;
  const displayCandles = candles.length > MAX_VISIBLE_CANDLES ? candles.slice(-MAX_VISIBLE_CANDLES) : candles;
  const hasCandles = candles.length > 0;
  const blockingCandleError = !hasCandles && snapshot?.candleError;
  ...
  const candleGap = 5;
  const candleW = displayCandles.length > 0 ? Math.max(5, Math.min(18, chartW / displayCandles.length - candleGap)) : 8;
  const axisValues = Array.from({ length: 6 }, (_, i) => high - (range * i) / 5);
  const targetY = typeof targetPrice === 'number' ? y(targetPrice) : null;

  return <section className="rounded-2xl border border-white/10 bg-[#11161B] p-6 text-[#EAECEF]"><div className="mb-4 flex items-center justify-between gap-3"><h2 className="text-2xl font-black tracking-[-0.04em] text-white">BTC Up or Down 15m</h2>{onRefresh ? <button onClick={onRefresh} className="rounded-full bg-slate-800 px-3 py-1 text-xs font-bold text-slate-200">Refresh</button> : null}</div><div className="flex flex-wrap gap-8"><div><div className="text-xs font-bold text-slate-500">Target Price</div><div className="text-2xl font-black text-slate-400">{typeof targetPrice === 'number' ? money(targetPrice) : 'Target unavailable'}</div></div><div className="border-l border-white/10 pl-8"><div className="text-xs font-bold text-slate-400">Final / Live Price</div><div className="text-2xl font-black text-white">{typeof livePrice === 'number' ? money(livePrice) : 'Live price unavailable'}</div></div><div className="border-l border-white/10 pl-8"><div className="text-xs font-bold text-slate-400">Distance</div><div className={typeof distance === 'number' ? (distance >= 0 ? 'text-2xl font-black text-emerald-400' : 'text-2xl font-black text-red-400') : 'text-2xl font-black text-slate-400'}>{typeof distance === 'number' ? `${distance >= 0 ? '▲' : '▼'} ${money(Math.abs(distance))}` : '—'}</div></div><div className="border-l border-white/10 pl-8"><div className="text-xs font-bold text-slate-400">Market Odds</div><div className="mt-1 flex gap-2 text-xs font-bold"><span className="rounded-full bg-emerald-500/15 px-3 py-1 text-emerald-300">UP {pct(snapshot?.outcomes.up.probability)}</span><span className="rounded-full bg-red-500/15 px-3 py-1 text-red-300">DOWN {pct(snapshot?.outcomes.down.probability)}</span></div></div></div>
    <div className="relative mt-4 h-[260px] overflow-hidden rounded-xl border border-slate-700/40 bg-[#11161B] lg:h-[280px]">{loading ? <div className="flex h-full items-center justify-center text-sm text-slate-400">Loading live candle data…</div> : error ? <div className="flex h-full items-center justify-center text-sm text-red-300">{error}</div> : blockingCandleError ? <div className="flex h-full items-center justify-center text-sm text-amber-300">Live candle source unavailable: {blockingCandleError}</div> : !hasCandles ? <div className="flex h-full items-center justify-center text-sm text-slate-400">Waiting for live candle data</div> : displayCandles.length < 2 ? <div className="flex h-full items-center justify-center text-sm text-slate-400">Waiting for more live candle data</div> : <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full" preserveAspectRatio="none">{axisValues.map((price) => { const yy = y(price); return <g key={price}><line x1={leftPad} x2={width - rightPad} y1={yy} y2={yy} stroke="rgba(71,85,105,0.45)" strokeWidth="1" /><text x={width - rightPad + 8} y={yy + 4} fill="rgba(148,163,184,0.75)" fontSize="11" fontWeight="700">{price.toFixed(2)}</text></g>; })}{targetY !== null ? <g><line x1={leftPad} x2={width - rightPad} y1={targetY} y2={targetY} stroke="#EF4444" strokeWidth="1" strokeDasharray="2 3" /></g> : null}{displayCandles.map((c, i) => { const x = leftPad + i * (chartW / Math.max(displayCandles.length - 1, 1)); const openY = y(c.open); const closeY = y(c.close); const highY = y(c.high); const lowY = y(c.low); const up = c.close >= c.open; const color = up ? '#22C55E' : '#EF4444'; const bodyY = Math.min(openY, closeY); const bodyH = Math.max(Math.abs(closeY - openY), 3); return <g key={`${c.timestamp}-${i}`}><line x1={x} x2={x} y1={highY} y2={lowY} stroke={color} strokeWidth="1.5" /><rect x={x - candleW / 2} y={bodyY} width={candleW} height={bodyH} fill={color} rx="1" /></g>; })}</svg>}</div>{candles.length > displayCandles.length ? <div className="mt-2 text-right text-[11px] text-slate-500">Showing last {displayCandles.length} of {candles.length} 1m candles</div> : null}
  </section>;
}
