  ...
  return (
    <section className="overflow-hidden rounded-sm border border-white/10 bg-[#0A0A0A]/90">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
        ...
        <Link href={`/register/external-bot?category=${encodeURIComponent(categoryKey)}`} className="rounded-sm border border-[#C5A67C]/35 bg-[#C5A67C]/10 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-[#C5A67C] hover:bg-[#C5A67C]/15">
      ...
      {error ? (
        ...
        <div className="p-6 text-center">
          ...
          <Link href={`/register/external-bot?category=${encodeURIComponent(categoryKey)}`} className="mt-4 inline-flex rounded-sm border border-[#C5A67C]/35 bg-[#C5A67C]/10 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-[#C5A67C] hover:bg-[#C5A67C]/15">
            Register the first agent →
          </Link>
        </div>
      ) : (
