...
describe('A2A jobs store', () => {
  ...
  it('createA2AJob returns deterministic-shaped record with status open', async () => {
    const result = await createA2AJob({ title: 'Test', description: 'Desc', requester: '0xabc' });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error('expected ok');
    expect(result.job.id).toMatch(/^job_[0-9a-f]{16}$/);
    expect(result.job.status).toBe('open');
    expect(result.job.title).toBe('Test');
    expect(result.job.requester).toBe('0xabc');
  });
  ...
  it('listA2AJobs filters by status, category, roleId', async () => {
    const r1 = await createA2AJob({ title: 'A', description: 'a', category: 'dev', roleId: 'r1' });
    const r2 = await createA2AJob({ title: 'B', description: 'b', category: 'pm', roleId: 'r2' });
  ...
  it('claimA2AJob locks job to first agent and rejects others', async () => {
    const created = await createA2AJob({ title: 'X', description: 'x' });
    expect(created.ok).toBe(true);
    if (!created.ok) throw new Error('expected ok');
    const first = await claimA2AJob(created.job.id, 'agent-1');
    expect(first.ok).toBe(true);
    if (first.ok) expect(first.job.status).toBe('claimed');

    const second = await claimA2AJob(created.job.id, 'agent-2');
    expect(second.ok).toBe(false);
    if (!second.ok) expect(second.error).toBe('job_already_claimed');

    // Idempotent for same agent
    const reclaim = await claimA2AJob(created.job.id, 'agent-1');
  ...
  it('submitA2AJob requires the claiming agentId', async () => {
    const created = await createA2AJob({ title: 'Y', description: 'y' });
    expect(created.ok).toBe(true);
    if (!created.ok) throw new Error('expected ok');
    await claimA2AJob(created.job.id, 'agent-1');
    const wrong = await submitA2AJob(created.job.id, { agentId: 'agent-2', output: 'x' });
    expect(wrong.ok).toBe(false);
    if (!wrong.ok) expect(wrong.error).toBe('agent_did_not_claim_job');

    const right = await submitA2AJob(created.job.id, { agentId: 'agent-1', output: 'done', proof: { type: 'signed_result' } });
  ...
  it('submitA2AJob auto-claims if job is still open', async () => {
    const created = await createA2AJob({ title: 'Z', description: 'z' });
    expect(created.ok).toBe(true);
    if (!created.ok) throw new Error('expected ok');
    const result = await submitA2AJob(created.job.id, { agentId: 'agent-9', output: 'done' });
    expect(result.ok).toBe(true);
    if (result.ok) {
  ...
  it('does not return a phantom job when DB insert fails', async () => {
    forceInsertError = 'insert failed';
    ...
    if (result.ok) {
      throw new Error('expected createA2AJob to fail');
    }

    expect(result.error).toBe('db_insert_failed');
    expect(result.detail).toContain('insert failed');
  });
});
