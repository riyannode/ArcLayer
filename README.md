...
| USDC | `0x3600000000000000000000000000000000000000` (Check `sdk/src/addresses.ts`) |
...
- `contracts/`: archived/experimental contracts and local test scaffolding; active Arc reference addresses live in sdk/src/addresses.ts.
...
- ArcLayer does not custody private keys.
- ArcLayer does not execute real trades.
- ArcLayer does not store model-provider secrets.
- External agents keep their own runtime, wallet, model credentials, and execution logic.
